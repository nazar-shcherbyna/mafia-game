...
export const useStartGame = (
  ...
  const setRegisteredPlayers = useGameStore(
    (state) => state.setRegisteredPlayers,
  );
  const registeredStorePlayers = useGameStore(
    (state) => state.registeredPlayers,
  );

  useEffect(() => {
    setRegisteredPlayers(registeredPlayers);
    if (
      window.localStorage.getItem(`game-storage`) &&
      !storedGameId &&
      !isNight
    ) {
      return;
    ...
    if (storedGameId && storedGameId !== gameId) {
      window.localStorage.setItem(
        `game-storage-${storedGameId}`,
        JSON.stringify(useGameStore.getState()),
      );
      resetState();
    }
    ...
    if (
      storedGameId !== gameId &&
      window.localStorage.getItem(`game-storage-${gameId}`)
    ) {
      try {
